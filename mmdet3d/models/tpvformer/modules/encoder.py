...
class TPVFormerEncoder(TransformerLayerSequence):
    ...
    def __init__(self,
        ...
        assert num_points_in_pillar[1] == num_points_in_pillar[2] and num_points_in_pillar[1] % num_points_in_pillar[
            0] == 0
    ...
    def point_sampling(self, reference_points, pc_range, cam_params, img_metas):

        rots, trans, intrins, post_rots, post_trans, bda = cam_params
        bs, num_cam, _ = trans.shape
        eps = 1e-5
        ogfH, ogfW = img_metas[0]['img_shape']
        reference_points = reference_points.clone()  # (B, num_points_in_pillar, num_query, 3)
        ...
        reference_points = reference_points[:, None].repeat(1, num_cam, 1, 1,
                                                            1)  # (B, num_cam, num_points_in_pillar, num_query, 3)

        reference_points = torch.inverse(bda).view(bs, 1, 1, 1, 3, 3).matmul(reference_points.unsqueeze(-1)).squeeze(
            -1)  # 1, 6, 4, 10000, 3

        reference_points -= trans.view(bs, num_cam, 1, 1, 3)  # 1, 6, 4, 10000, 3
        combine = rots.matmul(torch.inverse(intrins)).inverse()  # 1, 6, 3, 3
        reference_points_cam = combine.view(bs, num_cam, 1, 1, 3, 3).matmul(reference_points.unsqueeze(-1)).squeeze(-1)
        reference_points_cam = torch.cat([reference_points_cam[..., 0:2] / torch.maximum(
            reference_points_cam[..., 2:3], torch.ones_like(reference_points_cam[..., 2:3]) * eps),
                                          reference_points_cam[..., 2:3]], -1)

        reference_points_cam = post_rots.view(bs, num_cam, 1, 1, 3, 3).matmul(
            reference_points_cam.unsqueeze(-1)).squeeze(-1)
        reference_points_cam += post_trans.view(bs, num_cam, 1, 1, 3)

        reference_points_cam[..., 0] /= ogfW
        reference_points_cam[..., 1] /= ogfH

        tpv_mask = (reference_points_cam[..., 2:3] > eps)
        tpv_mask = (tpv_mask & (reference_points_cam[..., 0:1] > eps)
                    & (reference_points_cam[..., 0:1] < (1.0 - eps))
                    & (reference_points_cam[..., 1:2] > eps)
                    & (reference_points_cam[..., 1:2] < (1.0 - eps)))

        # (B, num_cam, num_points_in_pillar, num_query, 3)
        reference_points_cam = reference_points_cam.permute(1, 0, 3, 2,
                                                            4)  # num_cam, B, num_query, num_points_in_pillar, 3
        tpv_mask = tpv_mask.permute(1, 0, 3, 2, 4).squeeze(-1)

        return reference_points_cam[..., :2], tpv_mask  # 6, 1, 10000, 4, 2
    ...
    def forward(self,
        ...
        for ref_3d in ref_3ds:
            reference_points_cam, tpv_mask = self.point_sampling(
                ref_3d, self.pc_range, cam_params, kwargs['img_metas'])  # num_cam, bs, hw++, #p, 2
